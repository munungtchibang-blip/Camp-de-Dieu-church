...
export default function EventManager() {
  ...
  const handleWeeklyProgramGen = async () => {
    setSubmitting(true);
    try {
      // Fetch the standard weekly program from Firestore
      const snap = await getDocs(collection(db, 'weekly_program'));
      const weeklyEvents = snap.docs.map(doc => ({ id: doc.id, ...doc.data() }));

      if (weeklyEvents.length === 0) {
        toast.error("Définissez d'abord le programme standard dans l'onglet 'Programme Hebdo'");
        setSubmitting(false);
        return;
      }
      ...
      for (const item of weeklyEvents as any[]) {
        ...
        await addDoc(collection(db, 'events'), {
          ...
          location: item.location || "Temple Principal",
          imageUrl: '',
          createdAt: serverTimestamp()
        });
      }
      toast.success("Programme de la semaine généré !");
    } catch (error) {
      console.error("Generation error:", error);
      toast.error("Erreur de génération. Vérifiez vos permissions.");
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        ...
        toast.success("Événement ajouté au programme !");
      }
      resetForm();
    } catch (error) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white dark:bg-dark-card p-8 rounded-3xl shadow-xl border border-church-border dark:border-dark-border transition-colors duration-300">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Titre de l'Événement</label>
              <input 
                type="text"
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2">Fin (Optionnel)</label>
                <input 
                  type="datetime-local"
                  value={newEvent.end}
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">Type</label>
              <select 
                value={newEvent.type}
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">Lieu</label>
              <input 
                type="text"
            ...
            <div>
              <label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-2 px-1">Description</label>
              <textarea 
            ...
            <div className="flex gap-4">
              <button 
                type="submit"
                disabled={submitting}
                className="flex-1 mt-2 py-4 bg-church-blue text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg hover:bg-church-dark dark:hover:bg-church-blue/80 transition-all disabled:opacity-50 flex items-center justify-center gap-2"
              >
      ...
      <div className="bg-white dark:bg-dark-card p-8 rounded-3xl shadow-xl border border-church-border dark:border-dark-border transition-colors duration-300">
        <h2 className="text-xl font-black text-church-dark dark:text-white mb-8 uppercase tracking-tight">Calendrier des Activités</h2>
        
        {loading ? (
          <div className="py-12 text-center text-slate-400 dark:text-slate-600 uppercase font-black text-[10px] tracking-widest">
            Chargement...
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {events.map(event => (
              <div key={event.id} className="p-6 bg-slate-50 dark:bg-slate-800/30 rounded-[32px] border border-slate-100 dark:border-dark-border flex items-start justify-between group hover:border-church-blue transition-all">
                <div className="flex items-start gap-4">
                  <div className="w-12 h-12 bg-white dark:bg-slate-800 rounded-xl border border-slate-100 dark:border-dark-border flex flex-col items-center justify-center text-church-blue flex-shrink-0 shadow-sm overflow-hidden">
                    {event.imageUrl ? (
                  ...
                  <div>
                    <h3 className="font-black text-church-dark dark:text-white uppercase tracking-tight text-sm mb-1">{event.title}</h3>
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-500">
                        ...
                        {event.end && event.end !== event.start && <span className="text-church-blue ml-1">→ {event.end.replace('T', ' à ')}</span>}
                      </div>
                      {event.location && (
                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 dark:text-slate-500">
                          <MapPin size={10} />
                ...
                <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button 
                    ...
                    className="p-2 text-slate-300 dark:text-slate-600 hover:text-church-blue transition-colors"
                    title="Modifier"
                  >
                    <Edit2 size={16} />
                  </button>
